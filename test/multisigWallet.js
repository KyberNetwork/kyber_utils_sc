...
contract('MultisigWallet', function (accounts) {
  ...
  describe(`#constructor`, async () => {
    it('revert - invalid data', async () => {
      // duplicated owner
      await expectRevert(MultiSigWallet.new([accounts[0], accounts[0]], 1), 'invalid owner');
      await expectRevert(MultiSigWallet.new([accounts[0]], 0), 'invalid _required or ownerCount');
      await expectRevert(MultiSigWallet.new([], 1), 'invalid _required or ownerCount');
      await expectRevert(MultiSigWallet.new([accounts[1]], 2), 'invalid _required or ownerCount');
      let ownerLists = [];
      let maxCount = 50;
      for (let i = 0; i <= maxCount; i++) ownerLists.push(accounts[0]);
      // too many owner
      await expectRevert(MultiSigWallet.new(ownerLists, 2), 'invalid _required or ownerCount');
    });
  ...
  describe(`#submit transaction`, async () => {
    it('revert - sender is now an owner', async () => {
      await expectRevert(
        multisigWallet.submitTransaction([accounts[0]], [0], ['0x'], {from: accounts[8]}),
        'only an owner'
      );
    });
    ...
    it('revert - invalid data', async () => {
      // invalid data length
      await expectRevert(multisigWallet.submitTransaction([], [], [], {from: owners[0]}), 'invalid lengths');
      await expectRevert(
        multisigWallet.submitTransaction([accounts[0]], [], [], {from: owners[0]}),
        'invalid lengths'
      );
      await expectRevert(
        multisigWallet.submitTransaction([accounts[0]], [0], [], {from: owners[0]}),
        'invalid lengths'
      );
      await expectRevert(
        multisigWallet.submitTransaction([zeroAddress], [0], ['0x'], {from: owners[0]}),
    ...
    it('record correct data with no execution', async () => {
      ...
      let tx = await multisigWallet.submitTransaction(targets, values, callDatas, {from: owners[1]});
  ...
  describe(`#confirm transaction`, async () => {
    it('revert - sender is not an owner', async () => {
      await expectRevert(multisigWallet.confirmTransaction(0, {from: accounts[6]}), 'only an owner');
    });

    it('revert - transaction does not exist', async () => {
      await expectRevert(multisigWallet.confirmTransaction(0, {from: owners[0]}), 'transaction does not exist');
    });

    it('revert - already confirmed', async () => {
      await multisigWallet.submitTransaction([accounts[0]], [0], ['0x'], {from: owners[0]});
      await expectRevert(multisigWallet.confirmTransaction(0, {from: owners[0]}), 'only not confirmed');
    });
  ...
  describe(`#confirm transaction with data`, async () => {
    ...
    it('revert - invalid execution data', async () => {
      ...
      await expectRevert(
        multisigWallet.confirmTransactionWithData(0, [accounts[0], accounts[0]], [0, 0], ['0x', '0x'], {
          from: owners[1],
        }),
        'invalid lengths'
      );
      await expectRevert(
        multisigWallet.confirmTransactionWithData(0, [accounts[1]], [0], ['0x'], {from: owners[1]}),
        'invalid action'
      );
      await expectRevert(
        multisigWallet.confirmTransactionWithData(0, [accounts[0]], [1], ['0x'], {from: owners[1]}),
        'invalid action'
      );
      await expectRevert(
        multisigWallet.confirmTransactionWithData(0, [accounts[0]], [0], ['0x0000'], {from: owners[1]}),
        'invalid action'
      );
    });
    ...
    it('record correct data - with execution', async () => {
      await multisigWallet.submitTransaction([accounts[0]], [0], ['0x0000'], {from: owners[0]});
      let listConfirms = [owners[0]];
  ...
  describe(`#revoke confirmation`, async () => {
    it('revert - sender is not an owner', async () => {
      await expectRevert(multisigWallet.revokeConfirmation(0, {from: accounts[6]}), 'only an owner');
    });

    it('revert - not confirmed yet', async () => {
      await multisigWallet.submitTransaction([accounts[0]], [0], ['0x'], {from: owners[0]});
      await expectRevert(multisigWallet.revokeConfirmation(0, {from: owners[1]}), 'only confirmed');
    });
    ...
    it('revert - already executed', async () => {
      ...
      let txData = await multisigWallet.getTransaction(0);
      Helper.assertEqual(true, txData.executed);
      await expectRevert(multisigWallet.revokeConfirmation(0, {from: owners[0]}), 'only not executed');
    });
  ...
  describe(`#execute transaction`, async () => {
    it('revert - sender is not an owner', async () => {
      await expectRevert(multisigWallet.executeTransaction(0, [], [], [], {from: accounts[6]}), 'only an owner');
    });

    it('revert - not confirmed yet', async () => {
      await multisigWallet.submitTransaction([accounts[0]], [0], ['0x'], {from: owners[0]});
      await expectRevert(multisigWallet.executeTransaction(0, [], [], [], {from: owners[1]}), 'only confirmed');
    });

    it('revert - already executed', async () => {
      await multisigWallet.submitTransaction([accounts[0]], [0], ['0x'], {from: owners[0]});
      await multisigWallet.confirmTransactionWithData(0, [accounts[0]], [0], ['0x'], {from: owners[1]});
      await expectRevert(multisigWallet.executeTransaction(0, [], [], [], {from: owners[0]}), 'only not executed');
    });
    ...
    it('revert - invalid execution data', async () => {
      ...
      await expectRevert(multisigWallet.executeTransaction(0, [], [], [], {from: owners[1]}), 'invalid lengths');
      await expectRevert(
      ...
      await expectRevert(multisigWallet.executeTransaction(0, [], [0], ['0x'], {from: owners[1]}), 'invalid lengths');
      await expectRevert(
        multisigWallet.executeTransaction(0, [accounts[0], accounts[0]], [0, 0], ['0x', '0x'], {from: owners[1]}),
        'invalid lengths'
      );
      await expectRevert(
        multisigWallet.executeTransaction(0, [accounts[1]], [0], ['0x'], {from: owners[1]}),
        'invalid action'
      );
      await expectRevert(
        multisigWallet.executeTransaction(0, [accounts[0]], [1], ['0x'], {from: owners[1]}),
        'invalid action'
      );
      await expectRevert(
        multisigWallet.executeTransaction(0, [accounts[0]], [0], ['0x0000'], {from: owners[1]}),
        'invalid action'
      );
    });
  ...
  describe(`#internal updates`, async () => {
    it(`add owner`, async () => {
      await expectRevert(multisigWallet.addOwner(accounts[2], {from: accounts[0]}), 'only this address');
      multisigWallet = await MultiSigWallet.new(owners, 1);
    ...
    it(`remove owner`, async () => {
      await expectRevert(multisigWallet.removeOwner(accounts[2], {from: accounts[0]}), 'only this address');
      ...
      await multisigWallet.confirmTransaction(0, {from: owners[1]});
      tx = await multisigWallet.confirmTransactionWithData(0, [multisigWallet.address], [0], [data], {
        from: owners[2],
      });
      expectEvent(tx, 'OwnerRemoval', {
        owner: owners[1],
    ...
    it(`change requirement`, async () => {
      await expectRevert(multisigWallet.changeRequirement(1, {from: accounts[0]}), 'only this address');
      multisigWallet = await MultiSigWallet.new(owners, 1);
    ...
    it('test withdraw tokens', async () => {
      ...
      for (let i = 0; i < tokens.length; i++) {
        if (tokens[i] == ethAddress) {
          ...
          await multisigWallet.submitTransaction([receiver], [amount], ['0x'], {from: owners[0]});
          Helper.assertEqual(balanceMultisig.sub(amount), await Helper.getBalancePromise(multisigWallet.address));
          Helper.assertEqual(balanceReceiver.add(amount), await Helper.getBalancePromise(receiver));
        } else {
          ...
          await multisigWallet.submitTransaction([tokens[i]], [0], [data], {from: owners[0]});
          Helper.assertEqual(balanceMultisig.sub(amount), await token.balanceOf(multisigWallet.address));
          Helper.assertEqual(balanceReceiver.add(amount), await token.balanceOf(receiver));
        }
      }
    });
  });

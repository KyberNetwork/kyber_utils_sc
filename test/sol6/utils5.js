...
contract('utils5', function (accounts) {
  ...
  describe('check dst qty calculation', async function () {
    ...
    it('check dest qty calculation revert for > MAX_QTY.', async function () {
      ...
      await expectRevert(utils5.mockCalcDstQty(srcQty, srcDecimal, dstDecimal, rate), 'srcQty > MAX_QTY');
    });
    ...
    it('check dest qty calculation revert for > MAX_RATE.', async function () {
      ...
      let srcQty = MAX_QTY;
      await expectRevert(utils5.mockCalcDstQty(srcQty, srcDecimal, dstDecimal, rate), 'rate > MAX_RATE');
    });
  });
  ...
  describe('check src qty calculation', async function () {
    ...
    it('check src qty calculation revert for > MAX_RATE.', async function () {
      ...
      let dstQty = MAX_QTY.div(new BN(2));
      let rate = MAX_RATE.add(new BN(1));
      await expectRevert(utils5.mockCalcSrcQty(dstQty, srcDecimal, dstDecimal, rate), 'rate > MAX_RATE');
    });
    it('check src qty calculation revert for > MAX_QTY.', async function () {
      ...
      await expectRevert(utils5.mockCalcSrcQty(dstQty, srcDecimal, dstDecimal, rate), 'dstQty > MAX_QTY');
    });
  });
  ...
  describe('should check when decimals diff > 18 calc reverted.', async function () {
    ...
    it('cal dst qty dst - src > MAX_DECIMALS', async function () {
      await expectRevert(utils5.mockCalcDstQty(30, smallDecimal, bigDecimal, 1500), 'dst - src > MAX_DECIMALS');
    });

    it('cal dst qty src - dst > MAX_DECIMALS', async function () {
      await expectRevert(utils5.mockCalcDstQty(30, bigDecimal, smallDecimal, 1500), 'src - dst > MAX_DECIMALS');
    });

    it('cal src qty dst - src > MAX_DECIMALS', async function () {
      await expectRevert(utils5.mockCalcSrcQty(30, smallDecimal, bigDecimal, 1500), 'dst - src > MAX_DECIMALS');
    });
    it('cal src qty src - dst > MAX_DECIMALS', async function () {
      await expectRevert(utils5.mockCalcSrcQty(30, bigDecimal, smallDecimal, 1500), 'src - dst > MAX_DECIMALS');
    });
  });
  ...
  describe('test calc rate', async function () {
    ...
    it('test calc function revert when srcAmount > MAX_QTY', async function () {
      ...
      await expectRevert(utils5.mockCalcRateFromQty(srcQty, destQty, srcDecimal, destDecimal), 'srcAmount > MAX_QTY');
    });
    ...
    it('test calc function revert when destAmount > MAX_QTY', async function () {
      ...
      await expectRevert(utils5.mockCalcRateFromQty(srcQty, destQty, srcDecimal, destDecimal), 'destAmount > MAX_QTY');
    });
